...
class BlockChain:
    ...
    def create_genesis_block(self):
        ...
        new_block = Block('Hello World!', self.difficulty, 'sinker048', self.miner_rewards)
    ...
    def verify_blockchain(self):
        ...
        for idx, block in enumerate(self.chain):
            if self.get_hash(block, block.nonce) != block.hash:
                ...
                print('Error: Hash mot matched to previous_hash')
                return False
    ...
    def add_transaction(self, transaction, signature):
        ...
        public_key += '\n-----END RSA PUBLIC KEY-----\n'
        public_key_pks = rsa.PublicKey.load_pkcs1(public_key.encode('utf-8'))
        transaction_str = self.transaction_to_string(transaction)
        if transaction.fee + transaction.amounts > self.get_balance(transaction.sender):
            return False, 'Balance not enough!'
        try:
            # test sender
            rsa.verify(transaction_str.encode('utf-8'), signature, public_key_pks)
        ...
        with connection:
            ...
            while True:
                ...
                if message:
                    ...
                    response_byte = str(response).encode('utf-8')
                    connection.sendall(response_byte)
